...
def _is_regular_trading_hours(now_et: datetime) -> bool:
    ...
    Monday-Friday) -- 2026-08-26, AYI/AFG/AIT/CCEP incident. Deliberately does NOT
    check holidays (a holiday already has no real trading either way, so the
    weekday+time check alone already yields the correct answer for this specific
    use -- gating the stop-breach market-sell fallback below, not anything that
    needs full calendar precision).

    Used to gate _place_stop_only's stop-breach market-sell fallback specifically,
    NOT the exit-order maintenance window itself (_exit_order_maintenance_window_open
    in web/app.py, which deliberately DOES include the pre-open lead time so a fresh
    stop gets placed ahead of the open -- that's correct, unrelated behavior). The
    bug this guards against: a stop-placement rejection ("stop price must be...
    current price") during the pre-open lead time reflects Alpaca's own live NBBO at
    that moment, which for a thin/no-liquidity pre-market print can be wildly
    unrepresentative of where the stock will actually open -- confirmed live (AYI,
    AFG, 2026-08-26): both were market-sold in response to this rejection at
    ~7:45 AM ET, based on a stop price Alpaca's pre-market quote judged already
    breached, then the resulting market order sat queued until the real 9:30 AM
    open and filled $16+ ABOVE the level that had supposedly already been breached
    -- proof the pre-market "breach" was an artifact of unreliable pre-market
    pricing, not a genuine decline. "Already breached, sell now" is only a
    meaningful, trustworthy signal once the real trading session has actually
    started."""
    if now_et.weekday() >= 5:
        return False
        ...
        if stop_price is not None and stop_price > 0 and stop_shares > 0:
            ...
            try:
                ...
                if "stop price must be" in err_str and "current price" in err_str and not _is_regular_trading_hours(
                        self._now_et()):
                    # 2026-08-26, AYI/AFG incident -- this exact rejection ALSO fires
                    # during the exit-order maintenance window's pre-open lead time
                    # (deliberately included so a fresh stop gets placed ahead of the
                    # open -- see _exit_order_maintenance_window_open in web/app.py),
                    # not just right after the real 9:30 AM open the way the original
                    # 2026-07-16 "Auto-close market-open race" fix was validated
                    # against. Confirmed live: AYI and AFG were both market-sold at
                    # ~7:45 AM ET off a stop rejection based on Alpaca's own pre-market
                    # NBBO, then the resulting market order queued until the real open
                    # and filled $16+ ABOVE the level that had supposedly already been
                    # breached -- proof the pre-market "breach" reflected unreliable
                    # thin-liquidity pricing, not a genuine decline. Skip the fallback
                    # entirely here (stop_ok stays False) -- the existing
                    ...
                    logger.warning(
                        ...
                        "-- NOT market-selling; will retry once the market opens",
                        stop_price, ticker,
                    )
                    ...
                    # error DURING REAL TRADING HOURS means the position should already
                    # be sold, not left with an unplaceable stop.
                    #
